...
<div class="content-header">
	<div class="content-header-title">Folha Riograndense - Editar Conteúdo</div>
...
<div class="content-forms" style="height: 420px;">
	<form action="<?=current_url(); ?>" method="post" id="form_main">
		<div class="row">
			...
			<div class="span10">
				<input type="text" name="form_titulo" value="<?=$conteudo->titulo; ?>" maxlength="120" class="span10 validate[required]">
			</div>
		</div>
		<div class="row">
			<div class="span2">Categoria:</div>
			<div class="span2">
				<input type="text" name="form_categoria" value="<?=$conteudo->categoria; ?>" maxlength="60" class="span2">
			</div>
			<div class="span1">Filtro:</div>
			<div class="span5">
				<?php
					$lista = explode('-',$conteudo->filtro);
					foreach($filtros as $key => $rows){
						if(in_array($key,$lista)){
							echo '
								<label class="checkbox inline">
									<input type="checkbox" checked="true" value="',$key,'" name="form_filtros[]"> ',$rows,'
								</label>
							';
						} else {
							echo '
			...
			<div class="span1">
				<input type="text" name="form_ordem" value="<?=$conteudo->ordem; ?>" maxlength="3" id="ordem" class="span1 text-center validate[required]">
			</div>
		</div>
		<div class="row">
			<div class="span12">
				<textarea name="form_html" id="html"><?=$conteudo->html; ?></textarea>
			</div>
		</div>
